...
def display_event_details_by_id(event_id):
    try:
        with open("events.pkl", "rb") as file:
            found = False
            while True:
        ...
        messagebox.showerror("Error", "Event file not found!")
def display_client_details_by_id(client_id):
    try:
        with open("clients.pkl", "rb") as file:
            found = False
            print(f"Searching for client with ID: {client_id}")  # Debug: Show what ID is being searched.
            while True:
                try:
                    client = pickle.load(file)
                    # Ensure both are strings or both are integers before comparing:
                    ...
                    break
            if not found:
                print(f"No matching client found for ID: {client_id}")  # Debug: Confirm if no client is found
                messagebox.showerror("Error", "Client not found!")
    except FileNotFoundError:
        messagebox.showerror("Error", "Client file not found!")

# Function to display details of a supplier given ID number
def display_supplier_details_by_id(supplier_id):
    found = False
    try:
        with open("suppliers.pkl", "rb") as file:
            while True:
...
def display_guest_details_by_id(guest_id):
    try:
        with open("guests.pkl", "rb") as file:
            found = False
            while True:
                try:
                    guest = pickle.load(file)
                    if str(guest.guest_id) == guest_id:
...
def display_venue_details_by_id(venue_id):
    try:
        with open("venues.pkl", "rb") as file:
            found = False
            while True:
                try:
                    venue = pickle.load(file)
                    if str(venue.venue_id) == venue_id:
                        details = f"Name: {venue.name}\nID: {venue.venue_id}\nAddress: {venue.address}\nContact: {venue.contact}\nMin Guests: {venue.min_guests}\nMax Guests: {venue.max_guests}"
        ...
        messagebox.showerror("Error", "Venue file not found!")


def display_employee_details_by_id(employee_id):
    try:
        with open("employees.pkl", "rb") as file:
            found = False
            while True:
                try:
                    employee = pickle.load(file)
                    if str(employee.employee_id) == employee_id:
...
def display_details_by_category_and_id(category, entity_id):
    if not entity_id:
        messagebox.showerror("Error", "ID is required!")
        return
    # Assuming functions like display_employee_details_by_id are already defined and work similarly to the example provided
    if category == "Employee":
        display_employee_details_by_id(entity_id)
    elif category == "Guest":
        display_guest_details_by_id(entity_id)
    elif category == "Supplier":
        display_supplier_details_by_id(entity_id)
    elif category == "Client":
        display_client_details_by_id(entity_id)
    elif category == "Venue":
        display_venue_details_by_id(entity_id)
    elif category == "Event":
        display_event_details_by_id(entity_id)
    else:
...
def main():
    ...
    display_by_id_frame = tk.Frame(notebook)
    ...
    notebook.add(display_by_id_frame, text='Display details by ID')
    ...
    populate_display_by_id_frame(display_by_id_frame)


    root.mainloop()
def populate_display_by_id_frame(frame):
    # Label and dropdown for choosing the category
    tk.Label(frame, text="Select Category:").grid(row=0, column=0, padx=10, pady=5)
    categories = ["Employee", "Guest", "Supplier", "Client", "Venue", "Event"]
    ...
    # Label and entry for ID input
    tk.Label(frame, text="Enter ID:").grid(row=1, column=0, padx=10, pady=5)
    ...
    def submit_action():
        ...
        selected_option = category_var.get()
        display_details_by_category_and_id(selected_option, entity_id)

    tk.Button(frame, text="Display Details", command=submit_action).grid(row=2, column=0, columnspan=2, padx=10, pady=5)

    # Optionally, you might want to add a button to clear the ID entry field
    tk.Button(frame, text="Clear ID", command=lambda: id_entry.delete(0, tk.END)).grid(row=3, column=0, columnspan=2, padx=10, pady=5)
